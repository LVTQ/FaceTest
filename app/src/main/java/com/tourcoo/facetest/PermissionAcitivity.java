	...
	public void startActiviy() {
		if (mRequestPermission.isEmpty()) {
			...
			new Thread(new Runnable() {
				@Override
				public void run() {
					MyApplication app = (MyApplication) PermissionAcitivity.this.getApplicationContext();
					app.mFaceDB.loadFaces();
					PermissionAcitivity.this.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							mProgressDialog.cancel();
							Intent intent = new Intent(PermissionAcitivity.this, DetecterActivity.class);
